# build-controller-local.ps1
# Local build script for Controller project (MSVC, vcpkg, ZeroMQ)
# This version sets INCLUDE and LIB for local builds, auto-detects MSVC/SDK, and uses full cl.exe path.
...
# Local MSVC/SDK detection and environment setup

# Ensure MSVC environment is set up by calling vcvars64.bat
$vcvarsPath = "C:\Program Files\Microsoft Visual Studio\2022\Community\VC\Auxiliary\Build\vcvars64.bat"
if (Test-Path $vcvarsPath) {
    Write-Host "Setting up MSVC environment using vcvars64.bat..."
    ...
    Write-Host "vcvars64.bat not found at $vcvarsPath. Please run from the x64 Native Tools Command Prompt for VS."
    exit 1
}
